...
show_summary() {
  ...
  if [[ "$TEST_MODE" == true ]]; then
    ...
    echo -e "${YELLOW}╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${BOLD}The 'devstack' command won't work yet in this terminal.${NC}"
    echo -e "You need to update your PATH. Choose one option:"
    echo ""
    echo -e "${CYAN}Option A: Update current terminal (Quick)${NC}"
    echo -e "   ${YELLOW}export PATH=\"\$HOME/bin:\$PATH\"${NC}"
    echo -e "   ${YELLOW}devstack${NC}"
    echo ""
    echo -e "${CYAN}Option B: Update permanently and reload (Recommended)${NC}"
    echo -e "   ${YELLOW}source ~/.zshrc${NC}"
    echo -e "   ${YELLOW}devstack${NC}"
    echo ""
    echo -e "${CYAN}Option C: Open a new terminal${NC}"
    echo -e "   Close this terminal and open a new one, then run:"
    echo -e "   ${YELLOW}devstack${NC}"
    echo ""
    echo -e "${CYAN}Option D: Use full path (No PATH update needed)${NC}"
    echo -e "   ${YELLOW}$HOME/bin/devstack${NC}"
    echo ""
    echo -e "─────────────────────────────────────────────────────────────"
    echo ""
    ...
    echo ""
    echo -e "${BOLD}• Reload tmux config (if already in tmux):${NC}"
    echo -e "   ${YELLOW}tmux source-file ~/.tmux.conf${NC}"
    echo ""
    echo -e "${BOLD}• Start with Claude CLI:${NC}"
    echo -e "   ${YELLOW}devstack --claude${NC}"
    echo ""
    echo -e "${BOLD}• Create named instance:${NC}"
    echo -e "   ${YELLOW}devstack --instance=myproject${NC}"
    echo ""
    echo -e "${BOLD}• Customize later:${NC}"
    echo -e "   ${YELLOW}$SCRIPT_DIR/skills/customize.sh${NC}"
    echo ""

    # Try to verify if devstack is accessible
    echo -e "${BOLD}Verifying installation...${NC}"
    if [[ ":$PATH:" == *":$HOME/bin:"* ]]; then
      echo -e "   ${GREEN}✓${NC} ~/bin is in your PATH"
      echo -e "   ${GREEN}✓${NC} Run: ${YELLOW}devstack${NC}"
    else
      echo -e "   ${YELLOW}⚠${NC} ~/bin is not yet in your PATH for this session"
      echo -e "   ${YELLOW}→${NC} Run one of the options above first"
    fi
    echo ""
  fi
